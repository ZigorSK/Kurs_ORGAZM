...
int Controller::find_pipeline_comand_time(bool type_of_comand, bool type_of_operand, int num)
{
	...
	//¬ыполнение команды
	if (!type_of_comand)//команда второго типа, выполнр€етс€ за (4, 8, 16) тактов
	{
		switch (num)
		{
		case 0:
	...
	{
		count++;// оманда 1-го типа, выполн€етс€ за 1 такт
	}


	//«апись данных по адресу второго операнда
	if (!type_of_operand)
	{
		switch (num)
		{
...
Controller::Controller()
{
	...
	std::cout << "¬ведите количество циклов работы конвеера, дл€ получени€ более точной оценки среднего времени работы: " << std::endl;
	std::cin >> n;
	N = n;
	srand(time(NULL));
